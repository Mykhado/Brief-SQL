...
select v.nom_village , count(h.num_hab)  from habitant h 
join village v on v.num_village = h.num_village  
...
select  h.nom, count (distinct p.num_potion) from habitant h 
join absorber a on h.num_hab =a.num_hab 
join potion p on p.num_potion = a.num_potion  
group by h.nom ;
...
select h.nom  ,count(t.num_trophee)  from trophee t  
join habitant h on t.num_preneur = h.num_hab group by h.nom  
having count(t.num_trophee )> (select count(t.num_trophee)  from trophee t  join habitant h on t.num_preneur = h.num_hab where h.nom ='Obélix' ) ;
