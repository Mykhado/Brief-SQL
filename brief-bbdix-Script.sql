...
select p.num_potion , p.lib_potion ,p.constituant_principal ,p.formule  from potion p ;
...
select t .num_trophee, t.date_prise ,t.code_cat,h.nom  from trophee t  join habitant h on t.num_preneur = h.num_hab  ;
--8. Nom des habitants qui habitent à Aquilona. (7 lignes)
select  h.nom  from habitant h  join village v on v.num_village = h.num_village where v.nom_village  = 'Aquilona';
--9. Nom des habitants ayant pris des trophées de catégorie Bouclier de Légat. (2 lignes)
select h.nom from habitant h  join  trophee t on t.num_resserre = h.num_hab join categorie c on t.code_cat =c.code_cat where c.nom_categ ='Bouclier de Légat';
--10. Liste des potions (libellés) fabriquées par Panoramix : libellé, formule et constituantprincipal. (3 lignes)
select p.lib_potion from potion p join fabriquer f  on p.num_potion = f.num_potion join habitant h on f.num_hab = h.num_hab where h.nom ='Panoramix';
...
select distinct  h.nom  from habitant h   join absorber a   on h.num_hab = a.num_hab  join potion p  on  p.num_potion = a.num_potion join fabriquer f on f.num_potion = p.num_potion  where f.num_hab  = 3;
--13. Liste des habitants (noms) ayant absorbé une potion fabriquée par Amnésix. (7 lignes)
select distinct   h.nom  from habitant h   join absorber a   on h.num_hab = a.num_hab  join potion p  on  p.num_potion = a.num_potion join fabriquer f on f.num_potion = p.num_potion  join  habitant h2 on h2.num_hab = f.num_hab where h2.nom = 'Amnésix' ;
...
select h.nom  from habitant h join absorber a  on h.num_hab = a.num_hab join potion p on p.num_potion = a.num_potion where p.lib_potion ='Potion magique n°1' and a.date_a between'2052-02-01'and '2052-02-28';
...
select v.nom_village, r.nom_resserre  from village v join resserre r on  v.num_village = r.num_village order by r.superficie desc  ;


--***

--18. Nombre d'habitants du village numéro 5. (4)
select count(h.num_village) from  habitant h where h.num_village =5;
--19. Nombre de points gagnés par Goudurix. (5)
select sum( c.nb_points ) from categorie c join trophee t on c.code_cat = t.code_cat join habitant h  on h.num_hab = t.num_preneur where h.nom ='Goudurix';
...
select sum (a.quantite) from absorber a join potion p on p.num_potion = a.num_potion where p.lib_potion = 'Potion magique n°2';
...
select v.nom_village , count(h.num_hab)  from habitant h join village v on v.num_village = h.num_village  group by v.nom_village  ;
--24. Nombre de trophées par habitant (6 lignes)
select h.nom  , count(t.num_trophee)  from trophee t  join habitant h on t.num_preneur = h.num_hab  group by h.nom  ;
--25. Moyenne d'âge des habitants par province (nom de province, calcul). (3 lignes)
select p.nom_province  , avg(h.age)  from province p join village v on p.num_province = v.num_province join habitant h on h.num_village= v.num_village group by p.nom_province  ;

--26. Nombre de potions différentes absorbées par chaque habitant (nom et nombre). (9lignes)
select  h.nom, count (distinct p.num_potion) from habitant h join absorber a on h.num_hab =a.num_hab join potion p on p.num_potion = a.num_potion  group by h.nom ;


--27. Nom des habitants ayant bu plus de 2 louches de potion zen. (1 ligne)
select h.nom  from habitant h  join absorber a on h.num_hab =a.num_hab join potion p on p.num_potion = a.num_potion where p.lib_potion ='Potion Zen'and a.quantite >2;

--***
--28. Noms des villages dans lesquels on trouve une resserre (3 lignes)
select v.nom_village  from village v join resserre r on r.num_village = v.num_village ;
...
select h.nom  ,count(t.num_trophee)  from trophee t  join habitant h on t.num_preneur = h.num_hab group by h.nom  having count(t.num_trophee )> (select count(t.num_trophee)  from trophee t  join habitant h on t.num_preneur = h.num_hab where h.nom ='Obélix' ) ;
